...
const EQUIPMENT_LABELS: Record<string, string> = {
  ...
  ARBALETE: 'Arbalète',
  HACHE_DE_BATAILLE: 'Hache de bataille',
  BOUCLIER: 'Bouclier',
  CASQUE: 'Casque',
  COTTE_DE_MAILLES: 'Cotte de mailles',
  ARMURE_DE_PLATES: 'Armure de plates',
  POTION_DE_GUERISON: 'Potion de guérison',
  EAU_BENITE: 'Eau bénite',
  POTION_DE_FORCE: 'Potion de force',
  POTION_DE_DEFENSE: 'Potion de défense',
  HACHE_NAINE: 'Hache naine',
  ARMURE_SIR_RAGNAR: 'Armure de Sir Ragnar',
  EPEE_RUNIQUE: 'Épée runique',
  SORT_DE_GUERISON: 'Sort de guérison',
  BOUCLIER_RUNIQUE: 'Bouclier runique',
  SORT_BOULE_DE_FEU: 'Sort de boule de feu',
  AMULETTE_DE_RESISTANCE: 'Amulette de résistance',
  EPEE_DE_GLACE: 'Épée de glace',
...
export class HeroCard {
  ...
  getClassInfo() {
    return HERO_CLASS_INFO[this.hero().heroClass];
  }
  maxHp(): number {
    return HERO_CLASS_INFO[this.hero().heroClass].healthPoints;
  }
  maxSp(): number {
    return HERO_CLASS_INFO[this.hero().heroClass].spiritPoints;
  }
  isDead(): boolean {
    return this.liveState().hp === 0;
  }
  ...
  getEquipLabel(id: string): string {
